...
def f1_score(y_true, y_pred):
    ...
    return 2*((prc*rcl)/(prc+rcl+K.epsilon()))


def load_dataset(in_file, batch_size, n_points, num_classes):
    assert os.path.isfile(in_file), '[error] dataset path not found'

    shuffle_buffer = 500

    def _extract_fn(data_record):
        in_features = {'points': tf.io.FixedLenFeature([n_points * 6], tf.float32),
                       'labels': tf.io.FixedLenFeature([n_points * num_classes], tf.float32)}
    ...
    def _preprocess_fn(sample):
        ...
        points = tf.reshape(points, (n_points, 6))
        # points = tf.random.shuffle(points)
        label = tf.reshape(label, (n_points, num_classes))
        return points, label
