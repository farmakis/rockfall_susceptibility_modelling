...
def augment(points, xforms, range=None):
    ...
    jitter_data = range * tf.compat.v1.random_normal(tf.shape(points_xformed), name='jitter_data')  #  CHANGE v1
...
def find_duplicate_columns(A):
    N = A.shape[0]
    P = A.shape[1]
    indices_duplicated = np.empty((N, 1, P), dtype=np.int32)
    indices_duplicated.fill(1)
    for idx in range(N):
...
def compute_eigenvals(A):
    ...
    q = tf.compat.v1.trace(A) / 3  # (N, P)                                                      #  CHANGE v1
...
def inverse_density_sampling(points, k, sample_num):
    ...
    point_indices = tf.compat.v1.py_func(random_choice_2d, [sample_num, prob_matrix], tf.int32)  #  CHANGE v1
...
def batch_normalization(data, is_training, name):
    return tf.keras.layers.BatchNormalization(data, momentum=0.99, training=is_training,
                                              beta_regularizer=tf.keras.regularizers.l2(1.0),
                                              gamma_regularizer=tf.keras.regularizers.l2(1.0),
                                              name=name)


class Seperable_Conv2D(Layer):

    def __init__(
            self, name, filters, kernel_size, strides=(1, 1), padding='VALID', activation=tf.nn.elu, depth_multiplier=1,
            initializer=tf.compat.v1.glorot_normal_initializer(), regularizer=tf.keras.regularizers.l2(1.0),
            bn=True,
    ):
        super(Seperable_Conv2D, self).__init__()
    ...
    def build(self, input_shape):
        ...
        self.conv2d = tf.keras.layers.SeparableConv2D(
            ...
            activation=self.activation, depth_multiplier=self.depth_multiplier,
            ...
            name=self.tag, use_bias=not self.bn,
        )

        if self.bn:
            self.bn_layer = BatchNormalization(
                momentum=0.99, beta_regularizer=self.regularizer,
                gamma_regularizer=self.regularizer,
                name=self.tag + '_bn',
            )

        super(Seperable_Conv2D, self).build(input_shape)

    def call(self, inputs, training=True):

        out = self.conv2d(inputs)
        if self.bn: out = self.bn_layer(out, training=training)
        return out
...
#     conv2d = tf.keras.layers.SeparableConv2D(input, output, kernel_size=kernel_size, strides=(1, 1), padding='VALID',
#                                              activation=activation,
#                                              depth_multiplier=depth_multiplier,
#                                              depthwise_initializer=tf.compat.v1.glorot_normal_initializer(),
#                                              pointwise_initializer=tf.compat.v1.glorot_normal_initializer(),
...
class Depthwise_Conv2D(Layer):

    def __init__(
            self, kernel_size, name, depth_multiplier, filters=None, padding='VALID', activation=tf.nn.elu,
            regularizer=tf.keras.regularizers.l2(1.0), bn=True,
    ):
    ...
    def build(self, input_shape):
        ...
        self.conv2d = tf.keras.layers.SeparableConv2D(
            ...
            activation=self.activation, depth_multiplier=self.depth_multiplier,
            depthwise_regularizer=self.regularizer, pointwise_regularizer=self.regularizer,
            bias_initializer=None if self.bn else 'zeros',
            bias_regularizer=None if self.bn else self.regularizer,
            name=self.tag,
        )
        ...
        super(Depthwise_Conv2D, self).build(input_shape)
...
#                      reuse=None, with_bn=True, activation=tf.nn.elu):
#     conv2d = tf.keras.layers.SeparableConv2D(input, num_outputs=None, kernel_size=kernel_size, padding='VALID',
#                                              activation_fn=activation,
#                                              depth_multiplier=depth_multiplier,
#                                              weights_initializer=tf.compat.v1.glorot_normal_initializer(),
#                                              weights_regularizer=tf.keras.regularizers.l2(1.0),
#                                              biases_initializer=None if with_bn else tf.zeros_initializer(),
#                                              biases_regularizer=None if with_bn else tf.keras.regularizers.l2(1.0),
...
class Conv2D(Layer):
    ...
    def __init__(
            self, filters, kernel_size, name, strides=(1, 1), activation=tf.nn.elu, padding='VALID',
    ...
    def build(self, input_shape):
        ...
        self.conv2d = tf.keras.layers.Conv2D(
            ...
            padding=self.padding, activation=self.activation, kernel_initializer=self.initializer,
            kernel_regularizer=self.regularizer, name=self.tag, use_bias=not self.bn,
        )
...
#            reuse=None, with_bn=True, activation=tf.nn.elu):
#     conv2d = tf.keras.layers.Conv2D(input, output, kernel_size=kernel_size, strides=(1, 1), padding='VALID',
#                                     activation=activation,
...
class Dense(Layer):
    ...
    def __init__(
            self, name, units, strides=(1, 1), activation=tf.nn.elu,
            initializer=tf.compat.v1.glorot_normal_initializer(),
            regularizer=tf.keras.regularizers.l2(1.0), bn=True,
    ):
...
#     dense = tf.keras.layers.Dense(input, units=output, activation=activation,
#                                   kernel_initializer=tf.compat.v1.glorot_normal_initializer(),
#                                   kernel_regularizer=tf.keras.regularizers.l2(1.0),
#                                   reuse=reuse, name=name, use_bias=not with_bn)
