...
from custom_layers.layers import Pointnet_SA, Pointnet_FP
...
class PointNet(Model):
	...
	def init_network(self):
		...
		self.dense1 = Dense(8, activation=self.activation)

		self.dropout1 = Dropout(self.keep_prob)

		self.dense2 = Dense(self.num_classes, activation=tf.nn.sigmoid if self.num_classes == 2 else tf.nn.softmax)

	def forward_pass(self, input, training):

		l0_xyz = tf.slice(input, [0, 0, 0], [-1, -1, 3])
		l0_points = tf.slice(input, [0, 0, 3], [-1, -1, 3])
		# l0_xyz = input
		# l0_points = None
